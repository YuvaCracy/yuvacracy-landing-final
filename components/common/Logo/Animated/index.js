...
const AnimatedLogo = ({ bg, color = "", lineColor = "" }) => {
  const [emblaRef, emblaApi] = useEmblaCarousel(option, [Autoplay(true)]);
  const router = useRouter();
  const slideBgStyle = `  ${bg}   pl-[.5rem] font-montserrat`
  return (
    <div onClick={() => router.push('/')} className='flex flex-col gap-[.3rem] font-montserrat cursor-pointer ' >
      <div className={`relative ${color ? color : " text-primary "} `}>
        ...
        <p className=' absolute right-[-1.0rem] top-[-.3rem] font-openSans font-[600] text-[1rem]    ' >&reg;</p>
      </div>

      {/* Animation code  */}
      <div className='flex relative items-center justify-between w-[100%] ' >
        <div className={`h-[.25rem] w-[100%] max-w-[3.6rem] absolute ml-[.35rem]  ${lineColor ? lineColor : " bg-primary "} `} />
                ...
                <div className='  w-[100%] ' key={index}>
                  <div className="flex items-center justify-end animated-logo-embla__slide" >
                    <div className={slideBgStyle} >
                      <h2 className={`text-[.7rem] font-montserrat ${color ? color : " text-primary "} font-[600]`} >{data.title}</h2>
                    </div>
                  </div>
                </div>
              ))}
